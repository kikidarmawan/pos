<template>
  <div>
    <h1 class="text-3xl font-bold text-gray-900 mb-6">Laporan Transaksi Pelanggan</h1>
    ...
    <div class="card mb-6">
      <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
        <div>
          ...
          <select v-model="filters.payment_status" @change="loadReport" class="input">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
      <div class="card">
        <p class="text-sm text-gray-600 mb-1">Jumlah Pelanggan</p>
        ...
        <table class="w-full text-sm text-left border-collapse">
          <thead class="bg-gray-50 text-gray-600 text-xs uppercase sticky top-0 shadow-sm z-10">
            <tr>
              ...
              <th class="px-4 py-2.5 border-b font-semibold whitespace-nowrap text-right">Jumlah Transaksi</th>
              <th class="px-4 py-2.5 border-b font-semibold whitespace-nowrap text-center">Total Transaksi</th>
              <th class="px-4 py-2.5 border-b font-semibold whitespace-nowrap text-right">Jumlah Retur</th>
              <th class="px-4 py-2.5 border-b font-semibold whitespace-nowrap text-center">Total Retur</th>
              <th class="px-4 py-2.5 border-b font-semibold whitespace-nowrap text-right">Jumlah Belanja</th>
              <th class="px-4 py-2.5 border-b font-semibold whitespace-nowrap text-center">Total Belanja</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100">
            <tr v-for="(row, index) in data" :key="index" class="hover:bg-gray-50 transition-colors">
              ...
              <td class="px-4 py-2.5 text-right text-gray-600">{{ formatCurrency(row.jumlah_transaksi) }}</td>
              <td class="px-4 py-2.5 text-center">{{ row.total_transaksi }}</td>
              <td class="px-4 py-2.5 text-right text-red-500">{{ formatCurrency(row.jumlah_retur) }}</td>
              <td class="px-4 py-2.5 text-center text-red-500">{{ row.total_retur }}</td>
              <td class="px-4 py-2.5 text-right font-medium text-green-600">{{ formatCurrency(row.jumlah_belanja) }}</td>
              <td class="px-4 py-2.5 text-center font-medium">{{ row.total_belanja_count }}</td>
            </tr>
