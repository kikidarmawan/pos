<template>
  <div>
    <div class="flex justify-between items-center mb-6">
      <h1 class="text-3xl font-bold text-gray-900">Return Transaksi</h1>
      <router-link v-if="hasPermission('create_sales')" to="/sale-returns/create" class="btn btn-primary">
    ...
    <div class="card mb-6">
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <input v-model="filters.search" @input="onSearchInput" type="text" placeholder="Cari no. retur atau invoice..." class="input" />
    ...
    <div class="card overflow-hidden">
      ...
      <div v-else-if="!returns.data || returns.data.length === 0" class="p-8 text-center">
        <p class="text-gray-600 mb-4">Belum ada data retur.</p>
        <router-link v-if="hasPermission('create_sales')" to="/sale-returns/create" class="btn btn-primary">
          <PlusIcon class="w-5 h-5 mr-2" />
          Buat Retur Pertama
        </router-link>
          ...
          <table class="table">
            <thead>
              <tr>
                ...
                <th class="whitespace-nowrap">Invoice Penjualan</th>
      ...
      <div class="bg-white rounded-lg shadow-xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
        <div class="p-6">
          ...
          <dl class="grid grid-cols-2 gap-2 text-sm mb-4">
            <dt class="text-gray-500">Invoice Penjualan</dt>
            <dd>{{ selectedReturn.sale?.invoice_number }}</dd>
...
const loadReturns = async (page) => {
  ...
  try {
    ...
    const response = await api.get('/sale-returns', { params })
